...
export function buildPlayablePrompt(prompt: string, skeletonKey?: GameplaySkeletonKey) {
  return [
    ...
    "- Expose a lightweight window.__OPENGAME_DEBUG__ hook for validation when the game has active targets. It may return { gameState, score, lives, playfield, activeTargets }, where activeTargets include x/y/size and a reachable boolean for visible current targets.",
    "",
...
function buildRetryPrompt(prompt: string, errorMsg: string, log?: string | null, skeletonKey?: GameplaySkeletonKey) {
  return [
    ...
    "- Ensure mouse/touch gestures change visible game state for gesture-driven games; slice/drag/draw/aim games must respond to automated swipes through the playfield.",
    ...
    "- Add or preserve window.__OPENGAME_DEBUG__ for target-heavy games so validation can confirm current targets are visible and reachable.",
