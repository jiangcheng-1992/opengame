...
export function buildPlayablePrompt(prompt: string, skeletonKey?: GameplaySkeletonKey, contentType: ContentTypeValue = "GAME") {
  ...
  return [
    ...
    "- Expose a lightweight window.__OPENGAME_DEBUG__ hook for validation. It should return { gameState, score, lives, level, wave, round, maxLevel, maxWave, totalLevels, inputCoverage, playfield, activeTargets, targetHistory, solvable, allLevelsSolvable, levelPlans }, where inputCoverage marks keyboard/pointer/touch support, activeTargets contains inspectable target objects with x/y/size/reachable fields (not just strings or counts), targetHistory records actual hits/reaches, and levelPlans marks each designed level as solvable when applicable.",
...
function buildRetryPrompt(prompt: string, errorMsg: string, log?: string | null, skeletonKey?: GameplaySkeletonKey) {
  return [
    ...
    "- Add or preserve window.__OPENGAME_DEBUG__ so validation can confirm target reachability, input coverage, and level/wave/round progression. activeTargets must contain target coordinates/size/reachable fields, and targetHistory should record actual hit or reach events.",
